...
class XpathUtil:
                        ...
                        if element.has_attr(attr):
                            ...
                            if len(dom.xpath(locator)) == 1:
                                ...
                                if (
                                    ...
                                    #     % (
                                    #         guessable_element,
                                    #         variable_name.encode("utf-8").decode(
                                    #             "latin-1"
                                    #         ),
                                    #         locator.encode("utf-8").decode("latin-1"),
                                    #     )
                                    # )
                                    xpath_result["id"] = "{}_{}".format(
                                        guessable_element,
                                        variable_name.encode("utf-8").decode("latin-1"),
                                    )
                                    xpath_result["xpath"] = locator.encode(
                                        "utf-8"
                                    ).decode("latin-1")
                                    break
                                else:
                            ...
                            if element.getText() == button_text.strip():
                                locator = self.guess_xpath_button(
                                    guessable_element, "text()", element.getText()
                                )
                            else:
                                locator = self.guess_xpath_using_contains(
    ...
    def get_variable_names(self, element):
        ...
        if (
            (element.has_attr("id") and len(element["id"]) > 2)
            and bool(re.search(r"\d", element["id"])) == False
            and (
                "input" not in element["id"].lower()
                and "button" not in element["id"].lower()
            )
        ):
        ...
        # condition to check if the 'value' attribute exists and not having date and time values in it.
            ...
            if (element.has_attr("type")) and (
                ...
                if element.getText() != "":
                    self.variable_name = (
                        element["type"] + "_" + element.getText().strip().strip("_.")
                    )
                else:
                    self.variable_name = (
                        element["type"] + "_" + element["value"].strip("_.")
                    )
            else:
        ...
        # condition to check if the "placeholder" attribute exists and is not having any numerics in it.
            ...
            element.has_attr("placeholder")
            and bool(re.search(r"\d", element["placeholder"])) == False
        ):
        ...
        # and printing the variable name
            ...
            element["type"] not in ("text", "button", "radio", "checkbox", "search")
        ):
...
#     xpath_list = xpath_obj.generate_xpath(soup)
#     print(xpath_list)
